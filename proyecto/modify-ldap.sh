...
modificar_grupo() {
  ...
  if [ -n "$new_cn" ]; then
    ...
    # Cambiar el DN del grupo en el mismo archivo LDIF
    echo "dn: $group_dn" >> /tmp/modificar_group.ldif
    echo "changetype: modrdn" >> /tmp/modificar_group.ldif
    echo "newrdn: cn=$new_cn" >> /tmp/modificar_group.ldif
    echo "deleteoldrdn: 1" >> /tmp/modificar_group.ldif

    # Actualizar el DN del grupo para futuras modificaciones
    group_dn="cn=$new_cn,$DN_GROUPS"
  fi
